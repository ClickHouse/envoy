...
Network::FilterStatus ClickHouseFilter::onData(Buffer::Instance& data, [[maybe_unused]] bool end_stream)
{
    ...
    if (!client_packet_type.isReady())
        client_packet_type.onData(it);

    assert(client_packet_type.isReady() || !it);
    ...
    {
        if (client_packet_type.value != Client::Hello)
        {
            ENVOY_CONN_LOG(error,
                "ClickHouse from client: expected {}({}) got {}({})",
                read_callbacks_->connection(),
                Client::toString(Client::Hello), Client::Hello, Client::toString(client_packet_type.value), client_packet_type.value);

            client_hands_off = true;
            return Network::FilterStatus::Continue;
        }
        client_handshake.onData(it);
    
        assert(client_handshake.isReady() || !it);
